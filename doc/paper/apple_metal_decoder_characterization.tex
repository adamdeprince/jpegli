...
identical to CPU JPEGli.  That exactness retains JPEGli's perceptual advantage
at high quality: at Q95 4:4:4 it scores 89.389 SSIMULACRA2 and 1.054
Butteraugli, versus 88.524/1.147 for TurboJPEG.  Across a separate
...
bitstream.  Our quality study finds that JPEGli generally improves
SSIMULACRA2 at Q75 and above and leads both SSIMULACRA2 and Butteraugli for
high-quality 4:4:4.  Metal acceleration can therefore close the performance
gap while retaining a reconstruction behavior that users may prefer; an
approximate GPU kernel would give up part of the reason to accelerate JPEGli
in the first place.
...
Perceptual measurements compare each decoded JPEG with its original source
using SSIMULACRA2 (higher is better) and Butteraugli (lower is
better)~\cite{ssimulacra2,butteraugli}.  Since every Metal result equals CPU
JPEGli, it necessarily inherits the same perceptual score; these metrics
...
\begin{table}[t]
  \centering
  \caption{Perceptual scores for JPEGli-encoded files.  Each cell is
  SSIMULACRA2 / Butteraugli; higher/lower is better.  Metal equals CPU JPEGli.}
  \label{tab:quality}
  \scriptsize
  \resizebox{\columnwidth}{!}{%
  \begin{tabular}{@{}rrlll@{}}
    \toprule
    Q & Sampling & CPU JPEGli / Metal & TurboJPEG & ImageIO \\
    \midrule
...
Table~\ref{tab:quality} confirms that decoder choice can change perceptual
scores even when the compressed bytes are fixed.  There is no universal
low-quality winner: TurboJPEG leads several Q50 and Butteraugli entries.
JPEGli generally leads SSIMULACRA2 at Q75 and above, and leads both metrics for
high-quality 4:4:4.  At Q90 4:2:0, JPEGli raises SSIMULACRA2 from TurboJPEG's
81.295 to 81.996 and from ImageIO's 79.726, although TurboJPEG has a slightly
lower Butteraugli distance (2.650 versus 2.671).  At Q90 4:4:4, JPEGli leads
both comparisons at 84.687/1.643 versus 84.015/1.654 for TurboJPEG and
83.919/1.679 for ImageIO.  The Q95 4:4:4 separation grows to 89.389/1.054,
versus 88.524/1.147 and 88.316/1.189.
...
a GPU backend.  Table~\ref{tab:quality} changes that tradeoff: at Q90 and Q95,
especially 4:4:4, JPEGli's adaptive reconstruction improves the measured
perceptual result from the same compressed file.  Direct Metal reaches 631.2
MP/s on the Q90 baseline 4:2:0 CLIC pass while remaining byte-identical to
JPEGli, combining the desired reconstruction with incumbent-class speed.

This is not a claim that JPEGli wins every metric or quality level.  The Q50
results and Q90 4:2:0 Butteraugli result show why the paper reports both
metrics rather than selecting only favorable cells.  It is a narrower systems
claim: when an application chooses JPEGli's reconstruction, it need not pay
the full original CPU latency on a sufficiently large Apple-silicon image.
